...
class ReviewsController extends Controller
{
    ...
    public function index()
    {
        ...
        return Reviews::join('users', 'users.id','=','reviews.user_id')->get(['reviews.id', 'users.username','description','button','purpose','scores','upvotes'])->sortByDesc("upvotes")->skip(1);
    }
    ...
    public function upvote()
    {
        ...
        if(!$review){
            return response()->json(['message' => 'Failed upvotes review, data id not found', 'id' => request('id')],400);
        }
        
        $jmlh = $review->upvotes;
        $button = $review->button;
        
        if($button == 'upGray.svg'){
        ...
        }else if($button == 'upGreen.svg'){
            $downvote = Reviews::where('id', request('id'))->update(['upvotes' => $jmlh-1]);
        
            if($downvote){
                $button = Reviews::where('id', request('id'))->update(['button' => 'upGray.svg']);
                if($button){
                    return response()->json(['message' => 'Successfully downvotes review'],200);    
                }
            }else{
                return response()->json(['message' => 'Failed downvotes review', 'id' => request('id'), 'upvote' => $jmlh],400);
            }
        }
